...
% Søker bare for 2 meter frem her.
% deler opp i venstre og høyre liste
for i = 1:length(x)
    if points(i,1)<0 && points(i,2)<2000 % �nsker ikke � kutte liste mer, for out of range problem i l�kke
...
% alt i milli. 
door_threshold = 70; % How big norm represents a door? Hvor stor topp på deriviert
range_threshold = 1000; % How far is odomotry from a existing door? Se bare på dører som er innenfor der odom sier du er
search_range = 1000; % How far ahead should we look for doors? Kun gjort for å slippe indexfeil
door_distance = 2; % How close should the robot be to the door before is it denoted as detected? distanse fra første dørkarm
        ...
        if found_doors(i,3)==1 % Right door
            ...
            for n = 1:length(rightpoints(:,1))
                ...
                if rightpoints(n,2) < search_range
                    ...
                    norm_right2(n) = (rightpoints(n,1) - rightpoints(n+2,1))/rightpoints(n,2) - rightpoints(n+2,2);
